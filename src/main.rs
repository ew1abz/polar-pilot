...
async fn main(spawner: Spawner) -> ! {
    ...
    let spi = Spi::new(p.SPI1, p.PA5, p.PA7, p.PA6, p.DMA1_CH3, p.DMA1_CH2, Irqs, spi_cfg);
    ...
    let el_pwm =
        SimplePwm::new(p.TIM2, Some(el_step), None, None, None, Hertz(STEP_HZ), Default::default());

    let az_step = PwmPin::new(p.PA8, OutputType::PushPull);
    let az_pwm =
        SimplePwm::new(p.TIM1, Some(az_step), None, None, None, Hertz(STEP_HZ), Default::default());
    ...
    let uart =
        usart::Uart::new(p.USART2, p.PA15, p.PA2, p.DMA1_CH7, p.DMA1_CH6, Irqs, usart_cfg).unwrap();
