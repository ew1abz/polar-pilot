...
impl<C: Chip, SPI: SpiDevice> WiznetDevice<C, SPI> {
    ...
    pub async fn read_frame(&mut self, frame: &mut [u8]) -> Result<usize, SPI::Error> {
        ...
        let expected_frame_size: usize = {
            ...
            if raw < 2 {
                defmt::warn!("w5500 rx: bogus frame size {}, discarding", raw);
        ...
        // Cap to frame buffer length to avoid out-of-bounds slice
        ...
        if expected_frame_size > read_len {
            let mut skip_ptr = read_ptr.wrapping_add((expected_frame_size - read_len) as u16);
            core::mem::swap(&mut read_ptr, &mut skip_ptr);
        }
